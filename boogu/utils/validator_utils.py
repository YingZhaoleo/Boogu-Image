...
def get_device_validator(additional_types: Optional[List[str]] = None):
    ...
    Base supported formats: 'cpu', 'cuda', 'cuda:x', 'npu', or 'npu:x' (where x is an integer).
    ...
    def validate_device_format(value: str):
        ...
        # (cpu|cuda|npu) matches these exact words
        # |(cuda|npu):\d+ matches device type followed by one or more digits (\d+)
        if re.match(r"^(cpu|cuda|cuda:\d+|npu|npu:\d+)$", value):
            return value
        ...
        allowed_msg = (
            "'cpu', 'cuda', 'cuda:x', 'npu', 'npu:x' "
            "(where x is an integer like 'cuda:0' or 'npu:0')"
        )
        if additional_types:
...
def validate_device_and_offload_strategy_compatibility(
    ...
    if not re.match(r"^(cpu|cuda|cuda:\d+|npu|npu:\d+)$", device):
        return False
